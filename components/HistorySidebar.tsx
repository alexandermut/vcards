...
import { downloadVCard, generateContactFilename } from '../utils/vcardUtils';
...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({
  ...
  const handleExportCSV = () => {
    ...
    downloadCSV(csv, `vcard_export_${new Date().toISOString().slice(0,10)}.csv`);
  };

  const handleExportAllVCF = () => {
    const vcfContent = history.map(item => item.vcard).join('\n');
    downloadVCard(vcfContent, `vcard_backup_all_${new Date().toISOString().slice(0,10)}.vcf`);
  };
  ...
  const handleExportImages = async () => {
      ...
      zip.generateAsync({type:"blob"}).then((content) => {
          ...
          link.href = url;
          link.setAttribute('download', `vcard_scans_${new Date().toISOString().slice(0,10)}.zip`);
