...
import { Sparkles, RotateCcw, Code, FileText, UploadCloud, Undo2, PenLine } from 'lucide-react';
...
export const Editor: React.FC<EditorProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'text' | 'code' | 'enrich'>('text');
  ...
  const handleOptimizeClick = () => {
      if (activeTab === 'enrich') {
          // Send special payload for update mode
          onOptimize(`VCARD_UPDATE_REQUEST|${value}|${enrichText}`);
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2">
        ...
        <div className="flex gap-2 items-center self-end sm:self-auto">
            ...
            onClick={() => {
                ...
                if(activeTab === 'enrich') setEnrichText('');
                onReset();
            }}
          ...
          <button
            ...
            disabled={isOptimizing || (activeTab === 'enrich' && !enrichText.trim())}
            className={`text-xs flex items-center gap-1 px-3 py-1.5 rounded-md text-white font-medium transition-all shadow-sm ${
              isOptimizing || (activeTab === 'enrich' && !enrichText.trim())
        ...
        {activeTab === 'text' ? (
           ...
           />
            ...
            <div className="absolute inset-0 flex flex-col p-4">
                <textarea
                    className="flex-1 w-full h-full p-4 font-sans text-sm text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:focus:ring-blue-500/40 placeholder-slate-400 dark:placeholder-slate-600"
                    value={enrichText}
      ...
      <div className="px-4 py-2 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex justify-between">
        <span>
            {activeTab === 'text' 
                ...
                : activeTab === 'enrich'
                ? `${enrichText.length} ${t.chars}`
                : `${value.split('\n').length} ${t.lines}`
            }
        </span>
        <span className="uppercase">
            {activeTab === 'text' ? 'Regex Parser' : activeTab === 'enrich' ? 'AI Enrichment' : 'vCard 3.0'}
        </span>
      </div>
    </div>
  );
